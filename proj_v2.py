...
texture_ids = [
	"D:\\Computer-Graphics\\Imagenes\\4.jpg",
	"D:\\Computer-Graphics\\Imagenes\\5.jpg",
	"D:\\Computer-Graphics\\Imagenes\\6.jpg",
	"D:\\Computer-Graphics\\Imagenes\\7.jpg"
]
...
def draw_rect():
	...
	draw_color((1.,.4,.3),vertices[0])
	draw_color((.9,.9,.2),vertices[1])
	draw_color((.8,.3,1.),vertices[2])
	glColor3f(1., 1., 1.)
	...
	if image_indices[0] is not None:
		...
		draw_image(load_texture(texture_ids[next_image_index[0]]), vertices[0])
	if image_indices[1] is not None:
		# Proyección ortográfica
		glOrtho(-1, 1, -1, 1, -1, 1)
		# Dibuja el cuadrado con la imagen proyectada
		draw_image(load_texture(texture_ids[next_image_index[1]]), vertices[1])
	if image_indices[2] is not None:
		# Proyección ortográfica
		glOrtho(-1, 1, -1, 1, -1, 1)
		# Dibuja el cuadrado con la imagen proyectada
		draw_image(load_texture(texture_ids[next_image_index[2]]), vertices[2])
		...
		for i, square_points in enumerate(vertices):
			if point_inside_square(square_points, norm_x, norm_y):
				...
				if image_indices[i] is not None:
					# Obtiene el índice de la siguiente imagen en la lista
					
					next_image_index[i] = (image_indices[i] + 1) % len(texture_ids)
					print(next_image_index)
					# Carga y muestra la siguiente imagen en el cuadrado
					image_indices[i] = load_texture(texture_ids[next_image_index[i]])
				else:
					next_image_index[i] = 0
                	# Carga y muestra la imagen en el cuadrado
					image_indices[i] = load_texture(texture_ids[next_image_index[i]])
		glutPostRedisplay()
